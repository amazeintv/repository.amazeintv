...
def Mpop(url):
    Menu('[COLOR white]Currently Trending Movie Genres[/COLOR]','http://www.imdb.com/genre/?ref_=nv_ch_gr_3',999995,'','','','','','')
    base = 'http://imdb.com'
...
def Mpopgen(url):
    ...
    for a,b,rest in match:
        Menu('[COLOR white]'+b.upper()+'[/COLOR]',a,999994,'','','','','','')
...
def tv_show():
    ...
    addDir('[COLOR white][B]TV Shows List[/COLOR][/B]','http://www.tvmaze.com/shows',402,icon,FANART,'','','','','')
    addDir('[COLOR white]SEARCH TV Shows[/COLOR]','',405,icon,FANART,'','','','','')
